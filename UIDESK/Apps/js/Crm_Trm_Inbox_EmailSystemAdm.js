...
function Dropdown_Template() {
    var cmbDataSourceTemplate = $('#ComboTemplateName');
    $.ajax({
        type: "POST",
        url: "asmx/Crm_Trx_Ticket.asmx/UIDESK_TrmMasterCombo",
        data: "{TrxID:'0', TrxUserName: '" + $("#hd_sessionLogin").val() + "', TrxAction: 'UIDESK182'}",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (data) {
            ...
            for (i = 0; i < json.length; i++) {
                ...
                resultSourceTemplate = '<option value="' + json[i].ID + '">' + json[i].FORMAT + '</option>';
                cmbDataSourceTemplate.append(resultSourceTemplate);
...
function ChangeTemplateResponse(val) {
    var selectedText = $("#ComboTemplateName").find("option:selected").text();
    var selectedValue = $("#ComboTemplateName").val();
    $.ajax({
        type: "POST",
        url: "asmx/Crm_Trx_Ticket.asmx/UIDESK_TrmMasterCombo",
        data: "{TrxID:'" + selectedValue + "', TrxUserName: '" + $("#hd_sessionLogin").val() + "', TrxAction: 'UIDESK183'}",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (data) {
