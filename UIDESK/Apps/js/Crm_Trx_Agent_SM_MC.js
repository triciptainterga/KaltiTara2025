...
function loadSiteDataCards() {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            // Add click event to the site cards
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            const groupedUsers = filteredData.reduce((acc, user) => {
                const site = user.Site || "Pusat";
                if (!acc[site]) acc[site] = [];
                acc[site].push(user);
                return acc;
            }, {});

            // Render user notifications grouped by site
            Object.keys(groupedUsers).forEach(site => {
                const users = groupedUsers[site];
                users.forEach(user => {
                    ...
                    switch (user.channelid) {
                        case 5:
                            ...
                            Maxhandlenya = user.MaxHandleComment;
                            break;
                        case 9:
                            Channeltype = "Direct Message";
                            Maxhandlenya = user.MaxHandleComment;
                             ...
                             <div class="card border shadow-none">
                                 ...
                                 <div class="card-footer bg-transparent border-top d-flex">
                                    <div class="flex-grow-1">
                                        ...
                                        <span class="badge rounded-pill" style="background-color: #038edc; color: white; font-size: 14px; cursor: pointer;" onclick="ReleaseScheduleDK('${user.id}')">
                                            ${Maxhandlenya}
                                        </span>
                                    </div>
        ...
        success: function (data) {
            ...
            for (i = 0; i < json.length; i++) {
                ...
                resultUserNotification = '<div class="col-xl-3 col-sm-6">' +
                    ...
                    '</p>' +
                    //'<p class="text-muted text-truncate mb-0">' +
                    //'<span class="mdi mdi-circle text-' + ScheduleStateColor + ' me-2" onclick=ReleaseSchedule("' + json[i].id + '")></span>' +
                    //'</p>' +
                    '</div>' +
                    '<div class="flex-shrink-0 dropdown"> ' +
                    ...
                    '<div class="card-footer bg-transparent border-top d-flex">' +
                    '<div class="flex-grow-1">' +
                    '<span class="badge rounded-pill badge-soft-success font-size-12" style="cursor:pointer;" onclick=ReleaseSchedule("' + json[i].id + '")>' + json[i].maxhandle + '</span>' +
                    '</div>' +
                    '<div class="flex-shrink-0 ms-2">' +
                        ...
                        for (i = 0; i < json.length; i++) {
                            if (json[i].Result == "True") {
                                swal(
                                    ...
                                    window.location.href = "Crm_Trx_Agent_SM_MC.aspx";
                                });
                            } else {
                                swal(
                                    '',
    ...
    swal({
        ...
        .then((willDelete) => {
            if (willDelete) {
                ...
                //    },
                //    "data": {
                //        "token_agent": $('#ContentPlaceHolder1_TrxTokenMetaAgentSM').val(),
