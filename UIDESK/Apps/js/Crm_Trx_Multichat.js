    ...
    $.ajax({
        ...
        success: function (siteResponse) {
            ...
            $(".card-site").on("click", function () {
                ...
                LoadingUser(selectedSite); // Filter users based on selected site
            });
        },
        error: function (xhr) {
...
function LoadingUser(selectedSite) {
    $.ajax({
        ...
        success: function (data) {
            ...
            var filteredUsers = json.filter(user => user.Site === selectedSite);
            ...
            if (filteredUsers.length > 0) {
                filteredUsers.forEach(function (user) {
                    var Channel = user.ChannelName || "0";
                    var ScheduleStateColor = user.ScheduleState === "1" ? "success" : "danger";
                    var userNotificationHtml = `
        ...
        error: function (xmlHttpRequest, textStatus, errorThrown) {
            console.error("Error loading user data:", xmlHttpRequest.responseText);
...
function SearchingUser(ParameterID) {
    ...
    $.ajax({
        ...
        data: "{Type:'Multichat', Value: '" + KondisiData + "', UserName: '" + $("#hd_sessionLogin").val() + "'}",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (data) {
            var json = JSON.parse(data.d);
                    ...
                    $.ajax(settings).done(function (response) {
                        ...
                        if (response.success == true) {
                            ...
                            var form_data = JSON.stringify({
                                ...
                                TrxSosialMedia: $("#ComboSosialMedia").val(), TrxCampaign: accountID, TrxAccountName: $("#ComboAccount option:selected").text(),
                                MaxCampaign: "0", sm_type: "0"
                            });
                            $.ajax({
...
function checkToken() {
    $.ajax({
        type: "POST",
        url: "asmx/Crm_Trx_Agent_SM.asmx/UIDESK_TrmMasterCombo",
        data: "{TrxID:'" + $("#ContentPlaceHolder1_TrxID").val() + "', TrxUserName: '" + $("#hd_sessionLogin").val() + "', TrxAction: 'UIDESK238'}",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (data) {
            ...
            for (i = 0; i < json.length; i++) {
                ...
                updateAuxDatakelola(json[i].TokenMeta, "logout", json[i].CompanyToken)
