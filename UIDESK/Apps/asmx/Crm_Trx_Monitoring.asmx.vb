...
<ScriptService()>
<ToolboxItem(False)>
Public Class Crm_Trx_Monitoring1
    ...
    Public Function BRA_LoginMonitoring(ByVal UserName As String) As String
        Dim connstring As String = ConfigurationManager.ConnectionStrings("DefaultConnection").ConnectionString
        Dim dt As DataTable = New DataTable()

        Dim NameSP = "Exec BRA_LoginMonitoring"
        Dim ExecSP = "" & NameSP & " '" & UserName & "'"
        Try
            Using conn As SqlConnection = New SqlConnection(connstring)
                conn.Open()
                Dim sqlComm As SqlCommand = New SqlCommand("BRA_LoginMonitoring", conn)
    ...
    <WebMethod(EnableSession:=True)>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function BRA_UserMonitoring(ByVal UserID As String, ByVal UserName As String, ByVal Type As String, ByVal Action As String) As String
        Dim connstring As String = ConfigurationManager.ConnectionStrings("DefaultConnection").ConnectionString
        Dim dt As DataTable = New DataTable()

        Dim NameSP = "Exec BRA_UserMonitoring"
        Dim ExecSP = "" & NameSP & " '" & UserName & "'"
        Try
            Using conn As SqlConnection = New SqlConnection(connstring)
                conn.Open()
                Dim sqlComm As SqlCommand = New SqlCommand("BRA_UserMonitoring", conn)
    ...
    <WebMethod(EnableSession:=True)>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function BRA_UserMonitoringRelease(ByVal UserID As String, ByVal UserName As String, ByVal Type As String, ByVal Action As String) As String
        ...
        Try
            Using con As New SqlConnection(constr)
                ...
                sqlComm.CommandText = "BRA_UserMonitoring"
                sqlComm.CommandType = CommandType.StoredProcedure
                sqlComm.Parameters.AddWithValue("UserID", UserID)
                sqlComm.Parameters.AddWithValue("UserName", UserName)
                sqlComm.Parameters.AddWithValue("Type", Type)
        ...
        Catch ex As Exception
            ...
            strExec = "exec BRA_UserMonitoring '" & UserID & "','" & UserName & "','" & Type & "','" & Action & "'"
