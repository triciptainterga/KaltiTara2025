...
<ScriptService()>
<ToolboxItem(False)>
Public Class QA_TrxNotificationWindows
    ...
    Public Function FormAction(ByVal TrxHeaderID As String, ByVal TrxLevelUser As String, ByVal TrxUserName As String, ByVal TrxStatus As String, ByVal TrxType As String) As String
        ...
        Try
            Using con As New SqlConnection(constr)
                ...
                sqlComm.CommandText = "QM_TrxNotifikasiWindows"
                sqlComm.CommandType = CommandType.StoredProcedure
                sqlComm.Parameters.AddWithValue("TrxHeaderID", TrxHeaderID)
        ...
        Catch ex As Exception
            ...
            strExec = "exec QM_TrxNotifikasiWindows '" & TrxHeaderID & "','" & TrxLevelUser & "','" & TrxUserName & "','" & TrxStatus & "','" & TrxType & "'"
    ...
    <WebMethod(EnableSession:=True)>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function FormActionQuery(ByVal TrxUserName As String) As String
        Dim TrxUserNameXSS As String = AntiXssEncoder.HtmlEncode(TrxUserName.Trim, True)
        Dim listTickets As List(Of IndexCounting) = New List(Of IndexCounting)()
        Dim strExec As String = String.Empty
        Dim _PopupCount As String = String.Empty
        Dim constr As String = ConfigurationManager.ConnectionStrings("DefaultConnection").ConnectionString
        Try
            Using con As New SqlConnection(constr)
                Dim sqlComm As New SqlCommand()
                sqlComm.Connection = con
                sqlComm.CommandText = "QM_TrxNotifikasiWindowsCounting"
                sqlComm.CommandType = CommandType.StoredProcedure
                sqlComm.Parameters.AddWithValue("TrxUserName", TrxUserNameXSS)
                con.Open()
                sqldr = sqlComm.ExecuteReader()
                While sqldr.Read()
                    _PopupCount &= sqldr("PopupCount").ToString
                End While
                sqldr.Close()
                con.Close()
            End Using
        Catch ex As Exception
            Dim objectTickets As IndexCounting = New IndexCounting()
            objectTickets.Result = "False"
            objectTickets.TrxmsgSystem = ex.Message()
            listTickets.Add(objectTickets)
            strExec = "exec QM_TrxNotifikasiWindowsCounting " & "'" & TrxUserNameXSS & "'"
            LogError(HttpContext.Current.Session("UserName"), ex, strExec)
        Finally
            Dim objectTickets As IndexCounting = New IndexCounting()
            objectTickets.Result = "True"
            objectTickets.TrxmsgSystem = _PopupCount
            listTickets.Add(objectTickets)
            strExec = "exec QM_TrxNotifikasiWindowsCounting " & "'" & TrxUserNameXSS & "'"
            LogSuccess(HttpContext.Current.Session("UserName"), strExec)
        End Try
    ...
    <WebMethod(EnableSession:=True)>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function FormActionDetail(ByVal TrxUserName As String) As String
        Dim connstring As String = ConfigurationManager.ConnectionStrings("DefaultConnection").ConnectionString
        Dim dt As DataTable = New DataTable()
        Dim TrxUserNameXSS As String = AntiXssEncoder.HtmlEncode(TrxUserName.Trim, True)

        Dim NameSP = "Exec QM_TrxNotifikasiWindowsDetail"
        Dim ExecSP = "" & NameSP & " '" & TrxUserNameXSS & "'"
        Try
            Using conn As SqlConnection = New SqlConnection(connstring)
                conn.Open()
                Dim sqlComm As SqlCommand = New SqlCommand("QM_TrxNotifikasiWindowsDetail", conn)
                sqlComm.Parameters.AddWithValue("@TrxUserName", TrxUserNameXSS)
                sqlComm.CommandType = CommandType.StoredProcedure
