...
<ScriptService()>
<ToolboxItem(False)>
Public Class SCH_CreateAgent_Sesi1
    ...
    Public Function UIDESK_TrmMasterTransaction(ByVal TrxID As String, ByVal TrxUserName As String, ByVal TrxAction As String, ByVal TrxActionType As String) As String
        ...
        Dim NameSP = "Exec UIDESK_SCH_TrmMasterTransaction"
        Dim ExecSP = "" & NameSP & " '" & TrxID & "','" & TrxUserName & "','" & TrxAction & "','" & TrxActionType & "'"
        Try
            Using conn As SqlConnection = New SqlConnection(connstring)
                conn.Open()
                Dim sqlComm As SqlCommand = New SqlCommand("UIDESK_SCH_TrmMasterTransaction", conn)
    ...
    <WebMethod(EnableSession:=True)>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function UIDESK_SCH_InsertAgentSesi(ByVal TrxID As String, ByVal TrxUserName As String, ByVal TrxAction As String, ByVal TrxActionType As String) As String
        Dim connstring As String = ConfigurationManager.ConnectionStrings("DefaultConnection").ConnectionString
        Dim dt As DataTable = New DataTable()

        Dim NameSP = "Exec UIDESK_SCH_CreateAgentSesi"
        Dim ExecSP = "" & NameSP & " '" & TrxID & "','" & TrxUserName & "','" & TrxAction & "','" & TrxActionType & "'"
        Try
            Using conn As SqlConnection = New SqlConnection(connstring)
                conn.Open()
                Dim sqlComm As SqlCommand = New SqlCommand("UIDESK_SCH_CreateAgentSesi", conn)
                sqlComm.Parameters.AddWithValue("@TrxID", TrxID)
                sqlComm.Parameters.AddWithValue("@TrxUserName", TrxUserName)
                sqlComm.Parameters.AddWithValue("@TrxAction", TrxAction)
                sqlComm.Parameters.AddWithValue("@TrxActionType", TrxActionType)
    ...
    <WebMethod(EnableSession:=True)>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function UIDESK_SCH_TrxChangeSchedule(ByVal TrxID As String, ByVal TrxUserName As String, ByVal TrxAction As String, ByVal TrxActionType As String, ByVal Param1 As String, ByVal Param2 As String, ByVal Param3 As String, ByVal Param4 As String) As String
        Dim connstring As String = ConfigurationManager.ConnectionStrings("DefaultConnection").ConnectionString
        Dim dt As DataTable = New DataTable()

        Dim NameSP = "Exec UIDESK_SCH_TrxChangeSchedule"
        Dim ExecSP = "" & TrxID & " '" & TrxUserName & "','" & TrxAction & "','" & TrxActionType & "','" & Param1 & "','" & Param2 & "','" & Param3 & "','" & Param4 & "'"
        Try
            Using conn As SqlConnection = New SqlConnection(connstring)
                conn.Open()
                Dim sqlComm As SqlCommand = New SqlCommand("UIDESK_SCH_TrxChangeSchedule", conn)
                sqlComm.Parameters.AddWithValue("@TrxID", TrxID)
                sqlComm.Parameters.AddWithValue("@TrxUserName", TrxUserName)
                sqlComm.Parameters.AddWithValue("@TrxAction", TrxAction)
                sqlComm.Parameters.AddWithValue("@TrxActionType", TrxActionType)
    ...
    <WebMethod(EnableSession:=True)>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function InsertMasterTrxChangeSchedule(ByVal TrxID As String, ByVal TrxUserName As String, ByVal TrxAction As String, ByVal TrxActionType As String, ByVal Param1 As String, ByVal Param2 As String, ByVal Param3 As String, ByVal Param4 As String) As String
        Dim connstring As String = ConfigurationManager.ConnectionStrings("DefaultConnection").ConnectionString
        Dim ExecSP = "Exec UIDESK_SCH_INSERT_MASTER '" & TrxID & "','" & TrxUserName & "','" & TrxAction & "','" & TrxActionType & "','" & Param1 & "','" & Param2 & "','" & Param3 & "','" & Param4 & "'"

        Try
            Using conn As SqlConnection = New SqlConnection(connstring)
                conn.Open()
                Dim sqlComm As SqlCommand = New SqlCommand("UIDESK_SCH_INSERT_MASTER", conn)
                sqlComm.Parameters.AddWithValue("@TrxID", TrxID)
                sqlComm.Parameters.AddWithValue("@TrxUserName", TrxUserName)
                sqlComm.Parameters.AddWithValue("@TrxAction", TrxAction)
                sqlComm.Parameters.AddWithValue("@TrxActionType", TrxActionType)
                ...
                ' Execute the stored procedure (no data expected in return)
                sqlComm.ExecuteNonQuery()

                conn.Close()
            End Using
        Catch ex As Exception
            ' Log error (you can use your logging mechanism here)
            LogError(HttpContext.Current.Session("UserName"), ex, ExecSP)
            Return "{""status"":""Error"",""message"":""" & ex.Message & """}"
        Finally
            ' Log success or completion (you can use your logging mechanism here)
            LogSuccess(HttpContext.Current.Session("UserName"), ExecSP)
        End Try
    ...
    <WebMethod(EnableSession:=True)>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function UIDESK_SCH_TrxChangeSchedule_NonSession(ByVal TrxID As String, ByVal TrxUserName As String, ByVal TrxAction As String, ByVal TrxActionType As String, ByVal Param1 As String, ByVal Param2 As String, ByVal Param3 As String, ByVal Param4 As String) As String
        Dim connstring As String = ConfigurationManager.ConnectionStrings("DefaultConnection").ConnectionString
        Dim dt As DataTable = New DataTable()

        Dim NameSP = "Exec UIDESK_SCH_TrxChangeSchedule_NonSession"
        Dim ExecSP = "" & TrxID & " '" & TrxUserName & "','" & TrxAction & "','" & TrxActionType & "','" & Param1 & "','" & Param2 & "','" & Param3 & "','" & Param4 & "'"
        Try
            Using conn As SqlConnection = New SqlConnection(connstring)
                conn.Open()
                Dim sqlComm As SqlCommand = New SqlCommand("UIDESK_SCH_TrxChangeSchedule_NonSession", conn)
                sqlComm.Parameters.AddWithValue("@TrxID", TrxID)
                sqlComm.Parameters.AddWithValue("@TrxUserName", TrxUserName)
                sqlComm.Parameters.AddWithValue("@TrxAction", TrxAction)
                sqlComm.Parameters.AddWithValue("@TrxActionType", TrxActionType)
    ...
    <WebMethod(EnableSession:=True)>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function GetDataHistory(ByVal TrxID As String, ByVal Status As String, ByVal TrxAction As String, ByVal AgentId As String) As String

        Dim connstring As String = ConfigurationManager.ConnectionStrings("DefaultConnection").ConnectionString
        Dim dt As DataTable = New DataTable()
        Dim NameSP As String = "Exec UIDESK_SCH_GET_Scheduler_History"
        Dim sql As String = "" & NameSP & " '" & TrxID & "'"
        Try
            Using conn As SqlConnection = New SqlConnection(connstring)
                conn.Open()
                Dim sqlComm As SqlCommand = New SqlCommand("UIDESK_SCH_GET_Scheduler_History", conn)
                sqlComm.Parameters.AddWithValue("@TrxID", TrxID)
                sqlComm.Parameters.AddWithValue("@TrxAction", TrxAction)
    ...
    <WebMethod(EnableSession:=True)>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function UIDESK_SCH_ForecastData(ByVal StartDateNya As String, ByVal EndDateNya As String, ByVal StartTimeNya As String, ByVal EndTimeNya As String, ByVal IntervalNya As String, ByVal TrxAction As String, ByVal TrxActionType As String) As String
        Dim connstring As String = ConfigurationManager.ConnectionStrings("DefaultConnection").ConnectionString
        Dim dt As DataTable = New DataTable()

        Dim NameSP = "Exec UIDESK_SCH_ForecastData"
        Dim ExecSP = "" & StartDateNya & " '" & EndDateNya & "','" & StartTimeNya & "','" & EndTimeNya & "','" & IntervalNya & "','" & TrxAction & "','" & TrxActionType & "'"
        Try
            Using conn As SqlConnection = New SqlConnection(connstring)
                conn.Open()
                Dim sqlComm As SqlCommand = New SqlCommand("UIDESK_SCH_ForecastData", conn)
                sqlComm.Parameters.AddWithValue("@StartDateNya", StartDateNya)
                sqlComm.Parameters.AddWithValue("@EndDateNya", EndDateNya)
                sqlComm.Parameters.AddWithValue("@StartTimeNya", StartTimeNya)
                sqlComm.Parameters.AddWithValue("@EndTimeNya", EndTimeNya)
                sqlComm.Parameters.AddWithValue("@IntervalNya", IntervalNya)
                sqlComm.Parameters.AddWithValue("@TrxAction", TrxAction)
                sqlComm.Parameters.AddWithValue("@TrxActionType", TrxActionType)
                sqlComm.CommandType = CommandType.StoredProcedure
                Dim da As SqlDataAdapter = New SqlDataAdapter()
