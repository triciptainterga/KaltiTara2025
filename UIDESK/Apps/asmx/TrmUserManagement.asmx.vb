...
Public Class TrmUserManagement
    ...
    <WebMethod(EnableSession:=True)>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function ActionAgentType(ByVal ID As String, ByVal UserName As String, ByVal Value As String) As String
        ...
        Try
            Using con As New SqlConnection(constr)
                Dim sqlComm As New SqlCommand()
                sqlComm.Connection = con
                sqlComm.CommandText = "BRA_AgenType"
                sqlComm.CommandType = CommandType.StoredProcedure
        ...
        Catch ex As Exception
            ...
            strExec = "exec BRA_AgenType '" & ID & "','" & UserName & "','" & Value & "'"
            LogError(HttpContext.Current.Session("UserName"), ex, strExec)
        Finally
            Dim objectTickets As resultInsert = New resultInsert()
    ...
    <WebMethod(EnableSession:=True)>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function ActionAgentAlias(ByVal ID As String, ByVal UserName As String, ByVal Value As String, ByVal Action As String) As String
        'Dim listTickets As List(Of resultInsert) = New List(Of resultInsert)()
        'Dim strExec As String = String.Empty
        'Dim constr As String = ConfigurationManager.ConnectionStrings("DefaultConnection").ConnectionString
        'Try
        ...
        '        Dim sqlComm As New SqlCommand()
        '        sqlComm.Connection = con
        '        sqlComm.CommandText = "BRA_AgenAlias"
        '        sqlComm.CommandType = CommandType.StoredProcedure
        '        sqlComm.Parameters.AddWithValue("ID", ID)
        '        sqlComm.Parameters.AddWithValue("UserName", UserName)
        '        sqlComm.Parameters.AddWithValue("Value", Value)
        ...
        '    objectTickets.TrxmsgSystem = ex.Message()
        '    listTickets.Add(objectTickets)
        '    strExec = "exec BRA_AgenAlias '" & ID & "','" & UserName & "','" & Value & "','" & Action & "'"
        '    LogError(HttpContext.Current.Session("UserName"), ex, strExec)
        'Finally
        '    Dim objectTickets As resultInsert = New resultInsert()
        '    objectTickets.Result = "True"
        ...
        Try
            Using con As New SqlConnection(constr)
                ...
                ' Eksekusi stored procedure dan baca hasilnya
                Using sqldr As SqlDataReader = sqlComm.ExecuteReader()
                    If sqldr.HasRows Then
                        ' Membaca hasil jika ada
                        While sqldr.Read()
                            ' Mengambil hasil dari kolom Result
                            result = sqldr("Result").ToString()
                            ' Anda dapat menambahkan logika lebih lanjut jika diperlukan untuk memproses data lain
                        End While
                    Else
                        result = "DataTidakDitemukan" ' Jika tidak ada hasil
                    End If
                End Using
                con.Close()
            End Using
        Catch ex As Exception
            ' Menangani exception dan menambahkan log error
            Dim objectTickets As resultInsert = New resultInsert()
            objectTickets.Result = "False"
            objectTickets.TrxmsgSystem = ex.Message
            listTickets.Add(objectTickets)
            ...
            LogError(HttpContext.Current.Session("UserName"), ex, strExec)
        Finally
            ' Jika result adalah DataSudahAda, kirim pesan bahwa data sudah ada
            Dim objectTickets As resultInsert = New resultInsert()
            If result = "DataSudahAda" Then
                objectTickets.Result = "False"
                objectTickets.TrxmsgSystem = "Data Sudah Ada"
                listTickets.Add(objectTickets)
                strExec = "exec BRA_AgenAlias '" & ID & "','" & UserName & "','" & Value & "','" & Action & "'"
                LogSuccess(HttpContext.Current.Session("UserName"), strExec)
            ElseIf result = "DataTidakDitemukan" Then
                ' Jika data tidak ditemukan, beri tahu bahwa data tidak ada
                objectTickets.Result = "True"
                objectTickets.TrxmsgSystem = "Data Tidak Ditemukan"
                listTickets.Add(objectTickets)
                strExec = "exec BRA_AgenAlias '" & ID & "','" & UserName & "','" & Value & "','" & Action & "'"
                LogSuccess(HttpContext.Current.Session("UserName"), strExec)
            Else
                ' Jika data berhasil diproses
                objectTickets.Result = "True"
                objectTickets.TrxmsgSystem = "Data Berhasil"
                listTickets.Add(objectTickets)
                strExec = "exec BRA_AgenAlias '" & ID & "','" & UserName & "','" & Value & "','" & Action & "'"
                LogSuccess(HttpContext.Current.Session("UserName"), strExec)
            End If
