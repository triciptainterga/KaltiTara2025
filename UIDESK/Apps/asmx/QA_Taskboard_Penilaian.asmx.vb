...
<ScriptService()>
<ToolboxItem(False)>
Public Class QA_Taskboard_Penilaian1
    ...
    <WebMethod(EnableSession:=True)>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function QM_TrxHeaderCounting(ByVal QM_UserName As String, ByVal QM_TypeUser As String, ByVal QM_LevelUser As String, ByVal QM_GroupQA As String, ByVal QM_GroupAgent As String) As String
        Dim connstring As String = ConfigurationManager.ConnectionStrings("DefaultConnection").ConnectionString
        Dim QM_GroupQA_Variabel As String = String.Empty
        Dim QM_GroupAgent_Variabel As String = String.Empty
        If QM_GroupQA = "" Then
            QM_GroupQA_Variabel = "-"
        Else
            QM_GroupQA_Variabel = QM_GroupQA
        End If
        If QM_GroupAgent = "" Then
            QM_GroupAgent_Variabel = "-"
        Else
            QM_GroupAgent_Variabel = QM_GroupAgent
        End If
        Dim dt As DataTable = New DataTable()
        Dim sql As String = ""
        Dim NameSP = "Exec QM_TrxHeaderCounting"
        Dim ExecSP = "" & NameSP & " '" & QM_UserName & "','" & QM_TypeUser & "','" & QM_LevelUser & "','" & QM_GroupQA & "','" & QM_GroupAgent & "'"
        Try
            Using conn As SqlConnection = New SqlConnection(connstring)
                conn.Open()
                Dim sqlComm As SqlCommand = New SqlCommand("QM_TrxHeaderCounting", conn)
                sqlComm.Parameters.AddWithValue("@QM_UserName", QM_UserName)
                sqlComm.Parameters.AddWithValue("@QM_TypeUser", QM_TypeUser)
                sqlComm.Parameters.AddWithValue("@QM_LevelUser", QM_LevelUser)
                sqlComm.Parameters.AddWithValue("@QM_GroupQA", QM_GroupQA)
                sqlComm.Parameters.AddWithValue("@QM_GroupAgent", QM_GroupAgent)
                sqlComm.CommandType = CommandType.StoredProcedure
        ...
        '        sql = "exec QM_TrxHeaderCounting '" & QM_UserName & "','" & QM_TypeUser & "','" & QM_LevelUser & "','" & QM_GroupQA_Variabel & "','" & QM_GroupAgent_Variabel & "'"
    ...
    <WebMethod(EnableSession:=True)>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function QM_TrxHeaderTransaction(ByVal TrxUserName As String, ByVal TrxLevelUser As String, ByVal TrxStatus As String) As String
        Dim connstring As String = ConfigurationManager.ConnectionStrings("DefaultConnection").ConnectionString
        Dim dt As DataTable = New DataTable()

        Dim NameSP = "Exec QM_TrxHeaderTransaction"
        Dim ExecSP = "" & NameSP & " '" & TrxUserName & "','" & TrxLevelUser & "','" & TrxStatus & "'"
        Try
            Using conn As SqlConnection = New SqlConnection(connstring)
                conn.Open()
                Dim sqlComm As SqlCommand = New SqlCommand("QM_TrxHeaderTransaction", conn)
                sqlComm.Parameters.AddWithValue("@TrxUserName", TrxUserName)
    ...
    <WebMethod(EnableSession:=True)>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function QM_TrxIndikatorSLA(ByVal HeaderID As String, ByVal TrxUserName As String) As String
        Dim connstring As String = ConfigurationManager.ConnectionStrings("DefaultConnection").ConnectionString
        Dim dt As DataTable = New DataTable()

        Dim NameSP = "Exec QM_TrxIndikatorSLA"
        Dim ExecSP = "" & NameSP & " '" & HeaderID & "','" & TrxUserName & "'"
        Try
            Using conn As SqlConnection = New SqlConnection(connstring)
                conn.Open()
                Dim sqlComm As SqlCommand = New SqlCommand("QM_TrxIndikatorSLA", conn)
    ...
    <WebMethod(EnableSession:=True)>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function QM_TrxIndikatorHeaderSLA(ByVal HeaderID As String, ByVal TrxUserName As String) As String
        Dim connstring As String = ConfigurationManager.ConnectionStrings("DefaultConnection").ConnectionString
        Dim dt As DataTable = New DataTable()

        Dim NameSP = "Exec QM_TrxIndikatorHeaderSLA"
        Dim ExecSP = "" & NameSP & " '" & HeaderID & "','" & TrxUserName & "'"
        Try
            Using conn As SqlConnection = New SqlConnection(connstring)
                conn.Open()
                Dim sqlComm As SqlCommand = New SqlCommand("QM_TrxIndikatorHeaderSLA", conn)
                sqlComm.Parameters.AddWithValue("@HeaderID", HeaderID)
                sqlComm.Parameters.AddWithValue("@TrxUserName", TrxUserName)
                sqlComm.CommandType = CommandType.StoredProcedure
                Dim da As SqlDataAdapter = New SqlDataAdapter()
    ...
    <WebMethod(EnableSession:=True)>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function InsertApprovedSPVQA(ByVal UserName As String, ByVal TransaksiID As String, ByVal Interaction As String) As String
        Dim listTickets As List(Of resultInsert) = New List(Of resultInsert)()
        Dim strExec As String = String.Empty
        Dim constr As String = ConfigurationManager.ConnectionStrings("DefaultConnection").ConnectionString
        Try
            Using con As New SqlConnection(constr)
                Dim sqlComm As New SqlCommand()
                sqlComm.Connection = con
                sqlComm.CommandText = "QA_TrxApprovedHeaderID_SP"
                sqlComm.CommandType = CommandType.StoredProcedure
                sqlComm.Parameters.AddWithValue("UserName", UserName)
                sqlComm.Parameters.AddWithValue("TransaksiID", TransaksiID)
                sqlComm.Parameters.AddWithValue("Interaction", Interaction)
                con.Open()
                sqlComm.ExecuteNonQuery()
                con.Close()
            End Using
        Catch ex As Exception
            ...
            objectTickets.TrxmsgSystem = ex.Message()
            listTickets.Add(objectTickets)
            strExec = "exec QA_TrxApprovedHeaderID_SP '" & UserName & "','" & TransaksiID & "','" & Interaction & "'"
            LogError(HttpContext.Current.Session("UserName"), ex, strExec)
        Finally
            Dim objectTickets As resultInsert = New resultInsert()
