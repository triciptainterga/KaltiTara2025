...
<ScriptService()>
<ToolboxItem(False)>
Public Class KM_Feature
    Inherits System.Web.Services.WebService

    Dim sqlcom As SqlCommand
    Dim sqlcon As New SqlConnection(ConfigurationManager.ConnectionStrings("KnowledgeConnection").ConnectionString)
    ...
    Public Function BRA_KM_FAQ_Searching(ByVal ParamSearching As String) As String
        Dim connstring As String = ConfigurationManager.ConnectionStrings("KnowledgeConnection").ConnectionString
        Dim dt As DataTable = New DataTable()

        Dim NameSP = "Exec BRA_KM_FAQ_Searching"
        Dim ExecSP = "" & NameSP & " '" & ParamSearching & "'"
        Try
            Using conn As SqlConnection = New SqlConnection(connstring)
                conn.Open()
                Dim sqlComm As SqlCommand = New SqlCommand("BRA_KM_FAQ_Searching", conn)
                sqlComm.Parameters.AddWithValue("@ParamSearching", ParamSearching)
    ...
    <WebMethod(EnableSession:=True)>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function BRA_KM_Peraturan_Searching(ByVal ParamSearching As String) As String
        Dim connstring As String = ConfigurationManager.ConnectionStrings("KnowledgeConnection").ConnectionString
        Dim dt As DataTable = New DataTable()

        Dim NameSP = "Exec BRA_KM_Peraturan_Searching"
        Dim ExecSP = "" & NameSP & " '" & ParamSearching & "'"
        Try
            Using conn As SqlConnection = New SqlConnection(connstring)
                conn.Open()
                Dim sqlComm As SqlCommand = New SqlCommand("BRA_KM_Peraturan_Searching", conn)
                sqlComm.Parameters.AddWithValue("@ParamSearching", ParamSearching)
                sqlComm.CommandType = CommandType.StoredProcedure
                Dim da As SqlDataAdapter = New SqlDataAdapter()
                Dim ds As DataSet = New DataSet()
