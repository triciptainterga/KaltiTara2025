...
Public Class MultiChat
    ...
    Public Class resultInsert
        ...
        Public Property ResultOutput As String
        Public Property ResultMessage As String
        Public Property ResultAntrian As String
        Public Property TrxmsgSystem As String
    End Class
    Public Class ResultEndChat
        Public Property Result As String
        Public Property ResultMessage As String
    End Class
    ...
    Public Sub LogSuccess(ByVal agentName As String, strValue As String)
        ...
        Catch exX As Exception
            ...
            Dim pathXX As String = HttpContext.Current.Server.MapPath("~/apps/ErrorLog/" & agentName & "/" & DateTime.Now.ToString("ddMMyyyy") & "/" & DateTime.Now.ToString("ddMMyyyy") & ".txt")
    ...
    Public Function JamLayanan(ByVal Site As String, ByVal Channel As String) As String
        Dim connstring As String = ConfigurationManager.ConnectionStrings("DefaultConnection").ConnectionString
        Dim dt As DataTable = New DataTable()

        Dim TrxAction As String = "SELECT"
        Dim NameSP = "Exec BRA_JamLayanan"
        Dim ExecSP = "" & NameSP & " '" & Channel & "'"
        Try
            Using conn As SqlConnection = New SqlConnection(connstring)
                conn.Open()
                Dim sqlComm As SqlCommand = New SqlCommand("BRA_JamLayanan", conn)
    ...
    <WebMethod(EnableSession:=True)>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function DataSite() As String
        Dim connstring As String = ConfigurationManager.ConnectionStrings("DefaultConnection").ConnectionString
        Dim dt As DataTable = New DataTable()

        Dim NameSP = "Exec UIDESK_TrmDropdown"
        Dim ExecSP = "" & NameSP & " '0','DK','UIDESK204'"
        Try
            Using conn As SqlConnection = New SqlConnection(connstring)
                conn.Open()
                Dim sqlComm As SqlCommand = New SqlCommand("UIDESK_TrmDropdown", conn)
                sqlComm.Parameters.AddWithValue("@TrxID", "0")
                sqlComm.Parameters.AddWithValue("@TrxUserName", "DK")
                sqlComm.Parameters.AddWithValue("@TrxAction", "UIDESK204")
                sqlComm.CommandType = CommandType.StoredProcedure
                Dim da As SqlDataAdapter = New SqlDataAdapter()
                Dim ds As DataSet = New DataSet()
                da.SelectCommand = sqlComm
