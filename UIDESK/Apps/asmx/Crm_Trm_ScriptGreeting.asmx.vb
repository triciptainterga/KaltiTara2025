...
<ScriptService()>
<ToolboxItem(False)>
Public Class Crm_Trm_ScriptGreeting1
    ...
    <WebMethod(EnableSession:=True)>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function ActionScript(ByVal ID As String, ByVal Channel As String, ByVal Header As String, ByVal Status As String,
                                 ByVal Greeting As String, ByVal UserName As String, ByVal Action As String) As String
        ...
        If Action = "Insert" Or Action = "Update" Or Action = "Delete" Then
            Try
                Using con As New SqlConnection(constr)
                    ...
                    sqlComm.CommandText = "AHU_Uidesk_TrxGreetingScript"
                    sqlComm.CommandType = CommandType.StoredProcedure
                    sqlComm.Parameters.AddWithValue("ID", ID)
                    sqlComm.Parameters.AddWithValue("Channel", Channel)
                    sqlComm.Parameters.AddWithValue("Header", Header)
                    sqlComm.Parameters.AddWithValue("Status", Status)
                    sqlComm.Parameters.AddWithValue("Greeting", Greeting)
            ...
            Catch ex As Exception
                ...
                strExec = "exec AHU_Uidesk_TrxGreetingScript '" & ID & "','" & Channel & "','" & Header & "','" & Status & "','" & Greeting & "','" & UserNameXSS & "','" & Action & "'"
        ...
        Else
            Dim connstring As String = ConfigurationManager.ConnectionStrings("DefaultConnection").ConnectionString
            Dim dt As DataTable = New DataTable()
            Dim NameSP = "Exec AHU_Uidesk_TrxGreetingScript"
            Dim ExecSP = "'" & NameSP & "','" & ID & "','" & Channel & "','" & Header & "','" & Status & "','" & Greeting & "','" & UserNameXSS & "','" & Action & "'"
            Try
                Using conn As SqlConnection = New SqlConnection(connstring)
                    conn.Open()
                    Dim sqlComm As SqlCommand = New SqlCommand("AHU_Uidesk_TrxGreetingScript", conn)
                    sqlComm.Parameters.AddWithValue("@ID", ID)
                    sqlComm.Parameters.AddWithValue("@Channel", Channel)
                    sqlComm.Parameters.AddWithValue("@Header", Header)
                    sqlComm.Parameters.AddWithValue("@Status", Status)
