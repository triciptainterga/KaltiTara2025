...
Public Class TrmMailSystem
    ...
    <WebMethod(EnableSession:=True)>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function UIDESK_TrmMasterDropdownFilter(ByVal ID As String, ByVal Param As String, ByVal StartDate As String, ByVal Enddate As String, ByVal UserName As String, ByVal Action As String) As String
        Dim connstring As String = ConfigurationManager.ConnectionStrings("DefaultConnection").ConnectionString
        Dim dt As DataTable = New DataTable()

        Dim NameSP = "Exec UIDESK_TrmMasterDropdownFilter"
        Dim ExecSP = "" & NameSP & " '" & ID & "','" & Param & "','" & StartDate & "','" & Enddate & "','" & UserName & "','" & Action & "'"
        Try
            Using conn As SqlConnection = New SqlConnection(connstring)
                conn.Open()
                Dim sqlComm As SqlCommand = New SqlCommand("UIDESK_TrmMasterDropdownFilter", conn)
    ...
    <WebMethod(EnableSession:=True)>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function BRA_FollowUpEmail(ByVal TrxUserName As String, ByVal TrxEmailFrom As String, ByVal TrxTO As String, ByVal TrxSubject As String, ByVal TrxCC As String,
                                       ByVal TrxBody As String, ByVal TrxDirection As String, ByVal TrxEmailID As String, ByVal TicketNumber As String, ByVal TrxType As String) As String
        Dim listTickets As List(Of resultInsert) = New List(Of resultInsert)()
        Dim strExec As String = String.Empty
        Dim _Valuenya As String = String.Empty
        Dim _Result As String = String.Empty
        ...
        Dim DirectoryX As String = Path.Combine(Server.MapPath(_RootAttachmentOutboxEmail & "" & TrxEmailID))
        If Not System.IO.Directory.Exists(DirectoryX) Then
            System.IO.Directory.CreateDirectory(DirectoryX)
        Else
        End If
        ...
        Try
            Using con As New SqlConnection(constr)
                Dim sqlComm As New SqlCommand()
                sqlComm.Connection = con
                sqlComm.CommandText = "BRA_FollowUpEmail"
                sqlComm.CommandType = CommandType.StoredProcedure
                sqlComm.Parameters.AddWithValue("TrxUserName", TrxUsernameXSS)
                sqlComm.Parameters.AddWithValue("TrxEmailFrom", TrxEmailFrom)
                sqlComm.Parameters.AddWithValue("TrxTO", TrxTO)
        ...
        Catch ex As Exception
            ...
            objectTickets.TrxmsgSystem = ex.Message()
            listTickets.Add(objectTickets)
            strExec = "exec BRA_FollowUpEmail '" & TrxUsernameXSS & "','" & TrxEmailFrom & "','" & TrxTO & "','" & TrxSubjectXSS & "','" & TrxCC & "','" & TrxBody & "','" & TrxEmailID & "','" & TrxDirection & "','" & TicketNumber & "','" & TrxType & "'"
            LogError(HttpContext.Current.Session("UserName"), ex, strExec)
        Finally
            Dim objectTickets As resultInsert = New resultInsert()
            objectTickets.Result = "True"
    ...
    Public Function AHU_Uidesk_TrxAssignEmail(ByVal Access As String, ByVal IvcID As String, ByVal Agent As String, ByVal ReasonAssign As String, ByVal User As String) As String
        ...
        Try
            Using con As New SqlConnection(constr)
                ...
                sqlComm.Parameters.AddWithValue("LayananID", "1")
        ...
        Catch ex As Exception
            ...
            objectTickets.TrxmsgSystem = ex.Message()
            listTickets.Add(objectTickets)
            strExec = "exec AHU_Uidesk_TrxAssignEmail '" & Access & "','" & IvcID & "','" & Agent & "','" & ReasonAssign & "','1','" & User & "'"
            LogError(HttpContext.Current.Session("UserName"), ex, strExec)
        Finally
            Dim objectTickets As resultInsert = New resultInsert()
            objectTickets.Result = "True"
    ...
    <WebMethod(EnableSession:=True)>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function JamOperasionalSite(ByVal ID As String, ByVal Site As String, ByVal Channel As String, ByVal Hari As String, ByVal StartTime As String, ByVal EndTime As String, ByVal Status As String, ByVal UserName As String, ByVal Action As String) As String
        If Action = "TABLE" Or Action = "SELECT" Then
            Dim connstring As String = ConfigurationManager.ConnectionStrings("DefaultConnection").ConnectionString
            Dim dt As DataTable = New DataTable()
            Dim NameSP = "Exec UIDESK_TrxOfficeHourSiteChannel"
            Dim ExecSP = "" & NameSP & " '" & ID & "','" & Site & "','" & Channel & "','" & Hari & "','" & StartTime & "','" & EndTime & "','" & Status & "','" & UserName & "','" & Action & "'"
            Try
                Using conn As SqlConnection = New SqlConnection(connstring)
                    conn.Open()
                    Dim sqlComm As SqlCommand = New SqlCommand("UIDESK_TrxOfficeHourSiteChannel", conn)
                    sqlComm.Parameters.AddWithValue("@ID", ID)
                    sqlComm.Parameters.AddWithValue("@Site", Site)
                    sqlComm.Parameters.AddWithValue("@Channel", Channel)
                    sqlComm.Parameters.AddWithValue("@Hari", Hari)
                    sqlComm.Parameters.AddWithValue("@StartTime", StartTime)
                    sqlComm.Parameters.AddWithValue("@EndTime", EndTime)
        ...
        Else
            ...
            Try
                Using con As New SqlConnection(constr)
                    Dim sqlComm As New SqlCommand()
                    sqlComm.Connection = con
                    sqlComm.CommandText = "UIDESK_TrxOfficeHourSiteChannel"
                    sqlComm.CommandType = CommandType.StoredProcedure
                    sqlComm.Parameters.AddWithValue("ID", ID)
                    sqlComm.Parameters.AddWithValue("Site", Site)
                    sqlComm.Parameters.AddWithValue("Channel", Channel)
            ...
            Catch ex As Exception
                ...
                objectTickets.TrxmsgSystem = ex.Message()
                listTickets.Add(objectTickets)
                strExec = "exec UIDESK_TrxOfficeHourSiteChannel '" & ID & "','" & Site & "','" & Channel & "','" & Hari & "','" & StartTime & "','" & EndTime & "','" & Status & "','" & UserName & "','" & Action & "'"
                LogError(HttpContext.Current.Session("UserName"), ex, strExec)
            Finally
                Dim objectTickets As resultInsert = New resultInsert()
                objectTickets.Result = "True"
