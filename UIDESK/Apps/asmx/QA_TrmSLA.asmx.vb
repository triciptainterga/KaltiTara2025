...
Public Class QA_TrmSLA
    ...
    <WebMethod(EnableSession:=True)>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function QM_TrxSLA(ByVal TrxID As String, ByVal TrxSLA As String, ByVal TrxUserName As String, ByVal TrxAction As String) As String
        Dim listTickets As List(Of resultInsert) = New List(Of resultInsert)()
        Dim strExec As String = String.Empty
        Dim TrxSLAXSS As String = AntiXssEncoder.HtmlEncode(TrxSLA.Trim, True)
        ...
        Try
            Using con As New SqlConnection(constr)
                ...
                sqlComm.CommandText = "QM_TrxSLA"
                sqlComm.CommandType = CommandType.StoredProcedure
                sqlComm.Parameters.AddWithValue("TrxID", TrxID)
                sqlComm.Parameters.AddWithValue("TrxSLA", TrxSLAXSS)
        ...
        Catch ex As Exception
            ...
            strExec = "exec QM_TrxSLA '" & TrxID & "','" & TrxSLAXSS & "','" & TrxUsernameXSS & "','" & TrxAction & "'"
    ...
    <WebMethod(EnableSession:=True)>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function QM_TrxPeriodeSLA(ByVal TrxID As String, ByVal TrxPeriode As String, ByVal TrxDueDate As String, ByVal TrxUserName As String, ByVal TrxAction As String) As String
        Dim listTickets As List(Of resultInsert) = New List(Of resultInsert)()
        Dim strExec As String = String.Empty
        Dim TrxPeriodeXSS As String = AntiXssEncoder.HtmlEncode(TrxPeriode.Trim, True)
        Dim TrxDueDateXSS As String = AntiXssEncoder.HtmlEncode(TrxDueDate.Trim, True)
        Dim TrxUsernameXSS As String = AntiXssEncoder.HtmlEncode(TrxUserName.Trim, True)
        Dim constr As String = ConfigurationManager.ConnectionStrings("DefaultConnection").ConnectionString
        Try
            Using con As New SqlConnection(constr)
                Dim sqlComm As New SqlCommand()
                sqlComm.Connection = con
                sqlComm.CommandText = "QM_TrxPeriodeSLA"
                sqlComm.CommandType = CommandType.StoredProcedure
                sqlComm.Parameters.AddWithValue("TrxID", TrxID)
                sqlComm.Parameters.AddWithValue("TrxPeriode", TrxPeriodeXSS)
                sqlComm.Parameters.AddWithValue("TrxDueDate", TrxDueDateXSS)
                sqlComm.Parameters.AddWithValue("TrxUserName", TrxUsernameXSS)
                sqlComm.Parameters.AddWithValue("TrxAction", TrxAction)
                con.Open()
                sqlComm.ExecuteNonQuery()
        ...
        Catch ex As Exception
            ...
            objectTickets.TrxmsgSystem = ex.Message()
            listTickets.Add(objectTickets)
            strExec = "exec QM_TrxPeriodeSLA '" & TrxID & "','" & TrxPeriodeXSS & "','" & TrxDueDateXSS & "','" & TrxUsernameXSS & "','" & TrxAction & "'"
            LogError(strLogTime, ex, strExec)
        Finally
            Dim objectTickets As resultInsert = New resultInsert()
            objectTickets.Result = "True"
