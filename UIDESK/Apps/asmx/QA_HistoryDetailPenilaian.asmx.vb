...
<ScriptService()>
<ToolboxItem(False)>
Public Class QA_HistoryDetailPenilaian1
    ...
    Public Function BRA_QM_TrxHistoryTransaction(ByVal UserName As String, ByVal Agent As String,
                                                 ByVal Channel As String, ByVal Tahun As String, ByVal Bulan As String) As String
        Dim connstring As String = ConfigurationManager.ConnectionStrings("DefaultConnection").ConnectionString
        Dim dt As DataTable = New DataTable()

        Dim NameSP = "Exec BRA_QM_TrxHistoryTransaction"
        Dim ExecSP = "" & NameSP & " '" & UserName & "','" & Agent & "','" & Channel & "','" & Tahun & "','" & Bulan & "'"
        Try
            Using conn As SqlConnection = New SqlConnection(connstring)
                conn.Open()
                Dim sqlComm As SqlCommand = New SqlCommand("BRA_QM_TrxHistoryTransaction", conn)
    ...
    <WebMethod(EnableSession:=True)>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function BRA_QM_TrxHistoryTransaction_Call(ByVal UserName As String, ByVal Agent As String, ByVal Tahun As String, ByVal Bulan As String) As String
        Dim connstring As String = ConfigurationManager.ConnectionStrings("DefaultConnection").ConnectionString
        Dim dt As DataTable = New DataTable()

        Dim NameSP = "Exec BRA_QM_TrxHistoryTransaction_Call"
        Dim ExecSP = "" & NameSP & " '" & UserName & "','" & Agent & "','" & Tahun & "','" & Bulan & "'"
        Try
            Using conn As SqlConnection = New SqlConnection(connstring)
                conn.Open()
                Dim sqlComm As SqlCommand = New SqlCommand("BRA_QM_TrxHistoryTransaction_Call", conn)
                sqlComm.Parameters.AddWithValue("@UserName", UserName)
                sqlComm.Parameters.AddWithValue("@Agent", Agent)
                sqlComm.Parameters.AddWithValue("@Tahun", Tahun)
                sqlComm.Parameters.AddWithValue("@Bulan", Bulan)
