...
function getChannelAgent(TanggalStart, TanggalEnd, IDnya) {
    ...
    const data = {
        TrxID: IDnya,
        TrxUserName: `${TanggalStart};${TanggalEnd}`,
        TrxAction: 'LIST',
        TrxActionType: 'AGENTTOSHIFT'
    };

    fetch('asmx/SCH_CreateShifts.asmx/UIDESK_TrmMasterTransaction', {
        method: 'POST',
        headers: {
...
function viewJadwalAgent(AgentID,Name) {
    var json = "";
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            var endDate = new Date(e, o + 6, 0); // Akhir bulan ini// Mengonversi nama hari menjadi array event dengan tanggal yang diulang setiap minggu dalam bulan ini
            var s = [];
