...
Public Class QM_Report_Penilaian_Call_ScoreCard_Detail
    ...
    Private Sub btn_Submit_Click(sender As Object, e As EventArgs) Handles btn_Submit.Click
        ...
        Dim queryInsert As String = "exec [QM_Report_PenilaianCall_Detail] '" & Session("UserName") & "','" & Format(dt_strdate.Value, "yyyy-MM-dd") & "','" & Format(dt_endate.Value, "yyyy-MM-dd") & "', '" & descweekend & "'"
        ...
        Catch ex As Exception
            Response.Write(DirectCast(ex.Message() & "_exec [QM_Report_PenilaianCall_Detail] '" & Session("UserName") & "','" & Format(dt_strdate.Value, "yyyy-MM-dd") & "','" & Format(dt_endate.Value, "yyyy-MM-dd") & "'", String))
        End Try
        ASPxGridView1.DataBind()
    End Sub
    Private Sub ASPxGridView1_Init(sender As Object, e As EventArgs) Handles ASPxGridView1.Init
        descweekend = If(chkWeekend.Checked, "True", "False")
        tempTrxStaff.SelectCommand = "exec [QM_Report_PenilaianCall_Detail] '" & Session("UserName") & "','" & Format(dt_strdate.Value, "yyyy-MM-dd") & "','" & Format(dt_endate.Value, "yyyy-MM-dd") & "', '" & descweekend & "'"
    End Sub
    ...
    Private Sub btn_Export_Click(sender As Object, e As EventArgs) Handles btn_Export.Click
        ...
        Select Case casses
            Case "xlsx"
                ASPxGridViewExporter1.WriteXlsxToResponse("Report_PenilaianCall_Detail_" & DateTime.Now.ToString("yyyyMMddhhmmss"))
            Case "xls"
                ASPxGridViewExporter1.WriteXlsToResponse("Report_PenilaianCall_Detail_" & DateTime.Now.ToString("yyyyMMddhhmmss"))
            Case "rtf"
                ...
                ASPxGridViewExporter1.WriteRtfToResponse("Report_PenilaianCall_Detail_" & DateTime.Now.ToString("yyyyMMddhhmmss"))
            Case "pdf"
                ASPxGridViewExporter1.Landscape = True
                ASPxGridViewExporter1.LeftMargin = 35
                ASPxGridViewExporter1.RightMargin = 30
                'ASPxGridViewExporter1.ExportedRowType = DevExpress.Web.ASPxGridView.Export.GridViewExportedRowType.All
                ASPxGridViewExporter1.MaxColumnWidth = 108
                ASPxGridViewExporter1.WritePdfToResponse("Report_PenilaianCall_Detail_" & DateTime.Now.ToString("yyyyMMddhhmmss"))
            Case "csv"
                ASPxGridViewExporter1.WriteCsvToResponse("Report_PenilaianCall_Detail_" & DateTime.Now.ToString("yyyyMMddhhmmss"))
            Case "Word"
                ASPxGridViewExporter1.WriteDocxToResponse("Report_PenilaianCall_Detail_" & DateTime.Now.ToString("yyyyMMddhhmmss"))
        End Select
    End Sub
