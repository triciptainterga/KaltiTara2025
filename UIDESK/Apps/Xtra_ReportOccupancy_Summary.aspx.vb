...
Public Class Xtra_ReportOccupancy_Summary
    ...
    Private Sub btn_Submit_Click(sender As Object, e As EventArgs) Handles btn_Submit.Click
        ...
        Dim queryInsert As String = "exec [BRA_ReportOCR_Summary] '" & Session("UserName") & "','" & Format(dt_strdate.Value, "yyyy-MM-dd") & "','" & Format(dt_endate.Value, "yyyy-MM-dd") & "', '" & descweekend & "', '" & descchannel & "'"
        ...
        Catch ex As Exception
            Response.Write(DirectCast(ex.Message() & "_exec [BRA_ReportOCR_Summary] '" & Session("UserName") & "','" & Format(dt_strdate.Value, "yyyy-MM-dd") & "','" & Format(dt_endate.Value, "yyyy-MM-dd") & "'", String))
        End Try
    ...
    Private Sub ASPxGridView1_Init(sender As Object, e As EventArgs) Handles ASPxGridView1.Init
        descweekend = If(chkWeekend.Checked, "True", "False")
        descchannel = If(Channel.Checked, "True", "False")
        tempTrxStaff.SelectCommand = "exec [BRA_ReportOCR_Summary] '" & Session("UserName") & "','" & Format(dt_strdate.Value, "yyyy-MM-dd") & "','" & Format(dt_endate.Value, "yyyy-MM-dd") & "', '" & descweekend & "', '" & descchannel & "'"
    End Sub
    ...
    Private Sub btn_Export_Click(sender As Object, e As EventArgs) Handles btn_Export.Click
        ...
        Select Case casses
            Case "xlsx"
                ASPxGridViewExporter1.WriteXlsxToResponse("ReportOccupancySummary_" & DateTime.Now.ToString("yyyyMMddhhmmss"))
            Case "xls"
                ASPxGridViewExporter1.WriteXlsToResponse("ReportOccupancySummary_" & DateTime.Now.ToString("yyyyMMddhhmmss"))
            Case "rtf"
                ...
                ASPxGridViewExporter1.WriteRtfToResponse("ReportOccupancySummary_" & DateTime.Now.ToString("yyyyMMddhhmmss"))
            Case "pdf"
                ASPxGridViewExporter1.Landscape = True
                ASPxGridViewExporter1.LeftMargin = 35
                ASPxGridViewExporter1.RightMargin = 30
                'ASPxGridViewExporter1.ExportedRowType = DevExpress.Web.ASPxGridView.Export.GridViewExportedRowType.All
                ASPxGridViewExporter1.MaxColumnWidth = 108
                ASPxGridViewExporter1.WritePdfToResponse("ReportOccupancySummary_" & DateTime.Now.ToString("yyyyMMddhhmmss"))
            Case "csv"
                ASPxGridViewExporter1.WriteCsvToResponse("ReportOccupancySummary_" & DateTime.Now.ToString("yyyyMMddhhmmss"))
            Case "Word"
                ASPxGridViewExporter1.WriteDocxToResponse("ReportOccupancySummary_" & DateTime.Now.ToString("yyyyMMddhhmmss"))
        End Select
    End Sub
