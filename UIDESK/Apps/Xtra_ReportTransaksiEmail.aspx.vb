...
Public Class Xtra_ReportTransaksiEmail
    ...
    Private Sub ASPxGridView1_Init(sender As Object, e As EventArgs) Handles ASPxGridView1.Init
        '      tempTrxEmail.SelectCommand = "select case JENIS_EMAIL 
        'WHEN 'Team_leader' THEN 'Reply_Email'
        'WHEN 'Forward_Email' THEN 'Forward_Email'
        '      ELSE 
        ''Reply_Email'
        '      END AS alias_jenis_email,
        '      *, 10000 as Amount,dbo.udf_StripHTML(EBODY_HTML) as DescriptionNonHtml,
        '      agent=(select [NAME] from msuser where msuser.USERNAME=agent) from [4_Report_email] where Username='" & Session("UserName") & "'"

        tempTrxEmail.SelectCommand = "exec BRA_Report_Transaction_Email '" & Session("UserName") & "'"
    End Sub

    Private Sub ASPxGridView1_Load(sender As Object, e As EventArgs) Handles ASPxGridView1.Load
        '      tempTrxEmail.SelectCommand = "select select case JENIS_EMAIL 
        'WHEN 'Team_leader' THEN 'Reply_Email'
        'WHEN 'Forward_Email' THEN 'Forward_Email'
        '      ELSE 
        ''Reply_Email'
        '      END AS alias_jenis_email, *,10000 as Amount,dbo.udf_StripHTML(EBODY_HTML) as DescriptionNonHtml,
        '      agent=(select [NAME] from msuser where msuser.USERNAME=agent) from [4_Report_email] where Username='" & Session("UserName") & "'"
    ...
    Private Sub btn_Export_Click(sender As Object, e As EventArgs) Handles btn_Export.Click
        ...
        Select Case casses
            ...
            Case "Word"
                ASPxGridViewExporter1.WriteDocxToResponse("R_Email_" & DateTime.Now.ToString("yyyyMMddhhmmss"))
        End Select
    End Sub
